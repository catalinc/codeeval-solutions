...
/**
 * Solution for http://codeeval.com/open_challenges/48/
 *
 * Idea taken from http://en.wikipedia.org/wiki/Hungarian_algorithm
 *
 * This implementation uses Hungarian algorithm implementation provided by Konstantinos A. Nedas.
 * See the {@link HungarianAlgorithm} for copyright and additional details.
 */
public class discount_offers {

    private static Set<Character> setFrom(final String s) {
    ...
    private static Set<Character> VOWELS = setFrom("aeiouy");

    private static int countVowels(final String s) {
        return countLettersInSet(s, VOWELS);
    }

    private static Set<Character> CONSONANTS = setFrom("bcdfghjklmnpqrstvwxz");

    private static int countConsonants(final String s) {
        return countLettersInSet(s, CONSONANTS);
    }

    private static int countLettersInSet(final String s, final Set<Character> set) {
        int count = 0;
        for (int i = 0; i < s.length(); i++) {
    ...
    private static boolean isLetter(final char ch) {
        return VOWELS.contains(ch) || CONSONANTS.contains(ch);
    }

    private static int countLetters(final String s) {
        int count = 0;
        for (int i = 0; i < s.length(); i++) {
    ...
    private static double computeSuitabilityScore(final String customer, final String product) {
        double score;
        final int numLettersInCustomerName = countLetters(customer);
        final int numLettersInProductName = countLetters(product);
        if (numLettersInProductName % 2 == 0) {
            score = countVowels(customer) * 1.5;
        } else {
            score = countConsonants(customer);
        }
        if (gcd(numLettersInProductName, numLettersInCustomerName) != 1) {
            score *= 1.5;
        }
        return score;
    }
    ...
    private static double computeMaxScore(final String[] customers, final String[] products) {
        double[][] scores = new double[customers.length][products.length];
        for (int i = 0; i < customers.length; i++) {
            for (int j = 0; j < products.length; j++) {
                scores[i][j] = computeSuitabilityScore(customers[i], products[j]);
            }
        }
        if (scores.length > scores[0].length) {
            scores = HungarianAlgorithm.transpose(scores);
        }

        final int[][] assignment = HungarianAlgorithm.hgAlgorithm(scores, "max");
        double sum = 0;
        for (int i = 0; i < assignment.length; i++) {
            sum = sum + scores[assignment[i][0]][assignment[i][1]];
        }
        return sum;
    }
    ...
    public static void main(String[] args) throws IOException {
        if (args.length != 1) {
            System.err.println("usage: java " + discount_offers.class.getName() + " filename");
            System.exit(1);
        }
        ...
        try {
            ...
            while ((line = in.readLine()) != null) {
                ...
                final String[] customers = tokens[0].split(",");
                final String[] products = tokens[1].split(",");
                out.printf("%.2f\n", computeMaxScore(customers, products));
            }
        } finally {
            if (in != null) {
                in.close();
